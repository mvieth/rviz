...
private:
  ...
  void processMessage(const geometry_msgs::Vector3Stamped::ConstPtr& msg) override;

  // Storage for the list of visuals par each joint intem
  // Storage for the list of visuals.  It is a circular buffer where
  // data gets popped from the front (oldest) and pushed to the back (newest)
  boost::circular_buffer<boost::shared_ptr<Vector3Visual> > visuals_;

  // Property objects for user-editable properties.
  rviz::ColorProperty *force_color_property_;
  rviz::FloatProperty *alpha_property_, *force_scale_property_,
      *width_property_;
