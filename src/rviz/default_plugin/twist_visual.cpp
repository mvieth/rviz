...
TwistVisual::TwistVisual(Ogre::SceneManager* scene_manager, Ogre::SceneNode* parent_node)
{
  ...
  // is time to render.
  //
  // Here we create a node to store the pose of the TwistStamped's header frame
...
void TwistVisual::setTwist(const geometry_msgs::Twist& twist)
{
  Ogre::Vector3 linear(twist.linear.x, twist.linear.y, twist.linear.z);
  Ogre::Vector3 angular(twist.angular.x, twist.angular.y, twist.angular.z);
  setTwist(linear, angular);
}

void TwistVisual::setTwist(const Ogre::Vector3& linear, const Ogre::Vector3& angular)
{
...
void TwistVisual::setLinearColor(float r, float g, float b, float a)
{
  arrow_linear_->setColor(r, g, b, a);
}
// Color is passed through to the rviz object.
void TwistVisual::setAngularColor(float r, float g, float b, float a)
{
...
void TwistVisual::setLinearScale(float s)
{
  linear_scale_ = s;
}

void TwistVisual::setAngularScale(float s)
{
  angular_scale_ = s;
}

void TwistVisual::setWidth(float w)
{
  width_ = w;
}

void TwistVisual::setHideSmallValues(bool h)
{
  hide_small_values_ = h;
}


void TwistVisual::setVisible(bool visible)
{
